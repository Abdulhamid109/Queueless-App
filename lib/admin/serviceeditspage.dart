...
class _ServiceEditsPageState extends State<ServiceEditsPage> {
  ...
  Future getSingleService (String serviceID)async{
    try {
      final response = await http.get(Uri.parse("$BaseUrl/admin/getSingleService/$serviceID"),
      headers: {'Content-Type':'application/json'}
      );
      if(response.statusCode == 200){
        final jsonbody = jsonDecode(response.body);
...
if (servicesList.isEmpty) {
  return Scaffold(
    ...
    drawer: Admindrawer(),
    body: Center(
      child: Text("No associated services found!"),
    ),
  );
}
    return Scaffold(
                            ...
                            builder: (context) {
                              return AlertDialog(
                                ...
                                content: Padding(
                                  ...
                                  child: FutureBuilder(
                                    future: getSingleService(serviceData["_id"]),
                                    builder: (context, asyncSnapshot) {
                                      if (asyncSnapshot.connectionState ==
                                        ...
                                        editServiceName.text = asyncSnapshot.data!["name"].toString();
                                        // debugPrint("🟡Debug Data => ${asyncSnapshot.data.toString()}");
                                        editServiceCharge.text = asyncSnapshot.data!["AvgDurationPerCustomer"].toString();
                                        editCustomerDuration.text = asyncSnapshot.data!["ChargesPerService"].toString();
                                        return Column(
                                          ...
                                          children: <Widget>[
                                            ...
                                            TextField(
                                              controller: editCustomerDuration,
                                              decoration: InputDecoration(
                                                labelText: "Customer duration",
                                                enabledBorder: OutlineInputBorder(),
                                                focusedBorder: OutlineInputBorder(),
                                              ),
                                            ),
