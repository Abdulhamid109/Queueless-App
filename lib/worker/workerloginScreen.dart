...
class _WorkerloginscreenState extends State<Workerloginscreen> {
  ...
  Future<void> handleLogin() async {
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception(
          "Error => responsebody-${response.body} responseStatus - ${response.statusCode}",
        );
      }
    } catch (e) {
      print("Error => $e");
  ...
  Future<void> updateWorkerStatus(String email) async {
    try {
      ...
      if (response.statusCode != 200) {
        ...
        final resbody = jsonDecode(response.body);
        CherryToast.error(title: Text(resbody["error"]));
      }
    } catch (e) {
            ...
            children: [
              ...
              Card(
                ...
                child: Padding(
                  ...
                  child: Text(
                    "Tip: Worker has to Daily Login to the application in order start the working slot",
                  ),
                ),
              ),
